...
export default function ExcelComparison ()
{
  ...
  const paginatedData = matchingData.slice( ( currentPage - 1 ) * rowsPerPage, currentPage * rowsPerPage );

  const handleNextPage = () => setCurrentPage( ( prev ) => Math.min( prev + 1, Math.ceil( matchingData.length / rowsPerPage ) ) );
  ...
  const compareFiles = async () =>
  {
    ...
    {
      const worker = new Worker( new URL( "../../../../public/workers/compareWorker.js", import.meta.url ) );
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 bg-gray-100 p-4 rounded">
        ...
        <Button onClick={compareFiles} disabled={!fileOneData.length || !fileTwoData.length}>
          Compare Files
        </Button>
      </div>
      ...
      {matchingData.length > 0 && headers.length > 0 && (
        <>
          <Separator className="my-4" />
          <div className="mt-4 text-lg font-bold">Data Match: {matchPercentage}%</div>
          <TableSection title="Matching Data" data={paginatedData} headers={headers} />
          <Button onClick={handlePrevPage} disabled={currentPage === 1}>
            Previous
          </Button>
          <Button onClick={handleNextPage} disabled={currentPage * rowsPerPage >= matchingData.length}>
            Next
          </Button>
        </>
      )}
